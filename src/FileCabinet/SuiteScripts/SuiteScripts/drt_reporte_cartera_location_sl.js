		 ...
		 */
		 ...
		 function( ui, runtime, error, format, url, redirect, message, query, record, search) {
			 ...
			 function createForm (context) {
				 var scriptParams = context.request.parameters;
				 var locations = '3, 305, 306, 312, 307, 410, 423, 524, 509, 311, 313, 434, 315';
				 var locations2 = '3, 305, 306, 312, 307, 410, 423, 524, 509, 311, 313, 434, 315';
				 var form = ui.createForm({
					 title: "Reporte de Aging  de Cartera Vencida"
				 });

				 if (scriptParams.custpage_sk_list_locationId != 0){
				 ...
				 var params = {
					...
					custpage_disa_as_date_of2 : scriptParams.custpage_sk_as_date_of2 || '',
				 ...
				 var field = form.addField({
					 id: 'custpage_disa_as_date_of', type: ui.FieldType.DATE, label: "Fecha 1: "
				 });
				 ...
				 // Fecha AS of Date
				 var field = form.addField({
					id: 'custpage_disa_as_date_of2', type: ui.FieldType.DATE, label: "Fecha 2: "
				});
				field.setHelpText({help: "Fecha de Cartera"});
				field.updateDisplaySize({height : 60, width: 60});
				if(params.custpage_disa_as_date_of2 != ''){
					field.defaultValue = params.custpage_disa_as_date_of2;
				}else{
					var date = new Date();
					field.defaultValue = date;
				}
				...
				field = form.addField({
				 ...
				 sublist.addField({
					 id: 'custpage_disa_sublist_actual', type: ui.FieldType.TEXT, label: 'Mes 1 Actual'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				 sublist.addField({
					 id: 'custpage_disa_sublist_0_30', type: ui.FieldType.TEXT, label: 'Mes 1 0 - 30'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				 sublist.addField({
					 id: 'custpage_disa_sublist_30_60', type: ui.FieldType.TEXT, label: 'Mes 1 30 -60'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				 sublist.addField({
					 id: 'custpage_disa_sublist_60_90', type: ui.FieldType.TEXT, label: 'Mes 1 60 - 90'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				 sublist.addField({
					 id: 'custpage_disa_sublist_90', type: ui.FieldType.TEXT, label: 'Mes 1 > 90'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				 sublist.addField({
					 id: 'custpage_disa_sublist_total', type: ui.FieldType.TEXT, label: 'Mes 1 Total'
				 }).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});

				 sublist.addField({
					id: 'custpage_disa_sublist_actual2', type: ui.FieldType.TEXT, label: 'Mes 2 Actual'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				sublist.addField({
					id: 'custpage_disa_sublist_0_302', type: ui.FieldType.TEXT, label: 'Mes 2 0 - 30'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				sublist.addField({
					id: 'custpage_disa_sublist_30_602', type: ui.FieldType.TEXT, label: 'Mes 2 30 -60'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				sublist.addField({
					id: 'custpage_disa_sublist_60_902', type: ui.FieldType.TEXT, label: 'Mes 2 60 - 90'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				sublist.addField({
					id: 'custpage_disa_sublist_902', type: ui.FieldType.TEXT, label: 'Mes 2 > 90'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});
				sublist.addField({
					id: 'custpage_disa_sublist_total2', type: ui.FieldType.TEXT, label: 'Mes 2 Total'
				}).updateDisplayType({displayType : ui.FieldDisplayType.INLINE});

				 if(params.reload == 1){
					try{
						var getData = getAgingReport(params, 1 );		
						if ( getData && getData.length > 0 ) {
							...
							for ( var i = 0; i < getData.length; i++ ) {
								...
								if(top10 < 10){
									...
									if(records.actual == null){sublist.setSublistValue({id: 'custpage_disa_sublist_actual', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_actual', line: row, value: format.format({value:records.actual, type: format.Type.FLOAT})});}
									if(records.range0_30 == null){sublist.setSublistValue({id: 'custpage_disa_sublist_0_30', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_0_30', line: row, value: format.format({value:records.range0_30, type: format.Type.FLOAT})});}
					...
					try{
						var getData = getAgingReport(params, 2);		
						if ( getData && getData.length > 0 ) {
							var row = 0;
							var loc = 0;
							var temploc = '';
							...
							for ( var i = 0; i < getData.length; i++ ) {
								var records = getData[i];
								if(top10 < 10){
									if(records.actual == null){sublist.setSublistValue({id: 'custpage_disa_sublist_actual2', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_actual2', line: row, value: format.format({value:records.actual, type: format.Type.FLOAT})});}
									if(records.range0_30 == null){sublist.setSublistValue({id: 'custpage_disa_sublist_0_302', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_0_302', line: row, value: format.format({value:records.range0_30, type: format.Type.FLOAT})});}
									if(records.range30_60 == null){sublist.setSublistValue({id: 'custpage_disa_sublist_30_602', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_30_602', line: row, value: format.format({value:records.range30_60, type: format.Type.FLOAT})});}
									if(records.range60_90 == null){sublist.setSublistValue({id: 'custpage_disa_sublist_60_902', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_60_902', line: row, value: format.format({value:records.range60_90, type: format.Type.FLOAT})});}
									if(records.range90 == null){sublist.setSublistValue({id: 'custpage_disa_sublist_902', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_902', line: row, value: format.format({value:records.range90, type: format.Type.FLOAT})});}
									if(records.total == null){sublist.setSublistValue({id: 'custpage_disa_sublist_total2', line: row, value: 0});}else{sublist.setSublistValue({id: 'custpage_disa_sublist_total2', line: row, value: format.format({value:records.total, type: format.Type.FLOAT})});}
									row ++;
								}
								if(loc != records.location){
									/*if(loc != 0){
			 ...
			 function getAgingReport (params, month){
				log.debug('params',params);
				if(month == 1 ){
					var mydate = params['custpage_disa_as_date_of']
				}else {
					var mydate = params['custpage_disa_as_date_of2']
				}
						 ...
						 case 
							 when (OT.foreigntotal - (select case when sum(NTLL.foreignamount)>0 then sum(NTLL.foreignamount) else 0 end as TotalPaid  FROM NextTransactionLineLink AS NTLL INNER JOIN Transaction AS NT ON (NT.ID = NTLL.NextDoc) WHERE ( NTLL.PreviousDoc = OT.id) AND (NT.type='CustPymt'  OR NT.type='CustCred') AND NT.trandate<= to_date('${mydate}','dd/mm/yyyy'))) > 0 then (to_date('${mydate}','dd/mm/yyyy') - OT.duedate)
										   ...
										   when ((select max(NT.trandate) FROM NextTransactionLineLink AS NTLL INNER JOIN Transaction AS NT ON (NT.ID = NTLL.NextDoc) WHERE ( NTLL.PreviousDoc = OT.id) AND
											   (NT.type='CustPymt'  OR NT.type='CustCred') AND NT.trandate<= to_date('${mydate}','dd/mm/yyyy')) - duedate) < 0 then 0
													   else
											   ...
											   (NT.type='CustPymt'  OR NT.type='CustCred') AND NT.trandate<= to_date('${mydate}','dd/mm/yyyy')) - duedate)
								  end
						 end
				 end as daysoverdue,
				 case when 
			 ...
			 --OT.entity = 620 and
			 OT.id = TL.transaction and TL.subsidiary=2 and TL.location in (${params['custpage_disa_list_locationId']}) and OT.trandate <= to_date('${mydate}','dd/mm/yyyy')
				 ),
			 
				 agingrangesbyCustomer AS
						 (
				 SELECT 
					 a.almacen, a.location, a.aging, sum(AmountUnPaid) as amount
				 from 
					 mytemprable a
				 group by 
					a.almacen, a.location, a.aging
				 ),
				 ...
				 select distinct
						 x.almacen,
						 x.location,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location and y.aging = 'Actual') as Actual,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location  and y.aging = '0 - 30') as Range0_30,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location  and y.aging = '30 - 60') as Range30_60,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location  and y.aging = '60 - 90') as Range60_90,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location  and y.aging = 'mas90') as Range90,
						 (select sum(amount) from agingrangesbyCustomer y where y.location= x.location  and (y.aging = 'Actual' or y.aging = '0 - 30' or y.aging = '30 - 60' or y.aging = '60 - 90' or y.aging = 'mas90')) as Total
				...
				select 
					 location, 
					 almacen,
					 case when Actual is null then 0 else round(Actual/1000,0) end as Actual, 
					 case when Range0_30 is null then 0 else round(Range0_30/1000,0) end as Range0_30, 
					 case when Range30_60 is null then 0 else round(Range30_60/1000,0) end as Range30_60, 
					 case when Range60_90 is null then 0 else round(Range60_90/1000,0) end as Range60_90,
					 case when Range90 is null then 0 else round(Range90/1000,0) end as Range90, 
					 case when Total is null then 0 else round(Total/1000,0) end as Total 
				from 
					 finalranges 
					 where 
					 total is not null 
