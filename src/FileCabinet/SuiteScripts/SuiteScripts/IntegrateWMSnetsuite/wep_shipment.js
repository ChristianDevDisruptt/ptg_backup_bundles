        ...
        try {
            ...
            */
            for (var i_1 in context) {
                var fechaSurtido = context[i_1].shipmentDate;
                var formato1 = format.format({
                    value: new Date(fechaSurtido),
                    ...
                    for (var i_3 in context[i_1].shippings[i_2].shippingDetails) {
                        ...
                        if (folioTransaccion) {
                            ...
                            for (var i_4 in context[i_1].shippings[i_2].shippingDetails[i_3].inventoryHeader.inventoryDetails) {
								...
								log.debug('quantity_context', quantity_context);
                                if (
                                    lot_context &&
                                    !loteObj[lot_context]
                                ) {
            ...
            */
            ...
            var itemFullFilter = [];
            log.audit('objTransaction.saleso', objTransaction.saleso)
            ...
            log.audit('itemFullFilter', itemFullFilter)
            var idSearchITSO = drt_wep_lib.searchRecord(search.Type.INVENTORY_TRANSFER, itemFullFilter, idColumns);
            if(salesOrderFilter.length > 0){
                log.audit('cantidad_arreglo_so', salesOrderFilter.length)
            ...
            if(transferOrderFilter.length > 0){
                log.audit('cantidad_arreglo', transferOrderFilter.length)
            ...
            if (
                ...
                log.audit("Inicia Procesamiento TRNFRO:", objTransaction.trnfro);
                var pto = procesTransferOrder(objTransaction.trnfro, skuObj, loteObj, context)
                log.audit("Termina Procesamiento TRNFRO:", pto);
            ...
            log.error({
                title: 'Error Procesamiento _post',
                details: error
            });
        ...
        try {
            ...
            for (var nt in param_transaction) {
                ...
                var objBody = {
                    ...
                    trandate: param_transaction[nt].trandate
                };
                ...
                if (
                    ...
                    if (
						...
						log.audit('objLineInventory', objLineInventory)
                        if (
                            itso[nt]
                        ) {
                            log.audit({
                                title: 'itso[param_transaction[nt].id_netsuite] ' + itso[param_transaction[nt].id_netsuite],
                                details: JSON.stringify(itso)
                            });
                            log.error({
                                title: 'Existe una orden de transferencia',
                                details: JSON.stringify(itso[param_transaction[nt].id_netsuite])
                            });
                        } else {
                    ...
                    if (
                        ...
                        // respuesta.data.push(soif);
                      	log.audit('nweit.data', nweit.data);
                      	log.audit('nweit', nweit);
                        if (nweit.data) {
                            try {
                                var idInventoryTransfer = nweit.data.save({
                                    enableSourcing: false,
                                    ignoreMandatoryFields: true
                                }) || '';
                              	log.audit('idInventoryTransfer', idInventoryTransfer);
                                respuesta.data.push({
                                ...
                                if (saveSuccesShipment.success) {
                                    log.debug('Ok', 'guardado de registro con exito');
                                }
                            } catch (error) {
                                ...
                                if (ErrorShipment.success) {
                                    log.audit('Se guardo el Shipment:', JSON.stringify(ErrorShipment));
                                }
                            }
                        }

                        var idItemFulfillment = soif.data.save({
                            enableSourcing: false,
                            ignoreMandatoryFields: true
                        }) || '';
                        ...
                        if (idItemFulfillment) {
                            ...
                            if (vueltaExist.success) {
                                ...
                                log.audit('Se actualizó 2da. Vuelta:', JSON.stringify(ErrorShipment));
                            }
                        }
                    }
                } else {
            ...
            log.audit({
                title: 'Resultado procesSalesOrder',
                details: JSON.stringify(respuesta)
            });
            return respuesta;
        }
