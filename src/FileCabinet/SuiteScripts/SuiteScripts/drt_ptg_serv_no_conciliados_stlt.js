...
define(['N/ui/serverWidget', 'N/runtime', 'N/error', 'N/format', 'N/task', 'N/redirect', 'N/ui/message', './drt_ptg_serv_no_conciliados_lib', 'N/url', 'N/https', 'N/record'],
	...
	function (ui, runtime, error, format, task, redirect, message, lib, url, https, record) {
		...
		const scGeneralScript = 'customscriptcustomscript_ptg_create_opor';
		...
		function createForm(context) {
			...
			var params = {
				...
				mydateIni: oScript.mydateIni || '',
				mydateFin: oScript.mydateFin || '',
				lines: oScript.lines || 0,
				planta: oScript.planta || '',
			};
           ...
           var field2 = form.addField({
              id: 'custpage_sk_sublist_lines', type: ui.FieldType.INTEGER, label: 'Servicios a Conciliar:'
            });
            field2.updateDisplayType({displayType: ui.FieldDisplayType.INLINE});
            //field.defaultValue = row;

			//Fecha Inicio
			var field = form.addField({
				id: 'custpage_mydateini',
			...
			if(params.mydateIni.length > 0){
				field.defaultValue = params.mydateIni;
			} else {
          		var date = new Date();
			...
			if(params.mydateFin.length > 0){
				field.defaultValue = params.mydateFin;
			} else {
          		var date = new Date();
				field.defaultValue = date;
			}
				...
				if (getTransaction && getTransaction.length > 0) {
					...
					for (var i = 0; i < getTransaction.length; i++) {
						...
						sublist.setSublistValue({
							id: 'custpage_total',
							line: row,
							value: format.format({value:parseFloat((records.price * records.litros).toFixed(2)), type: format.Type.CURRENCY})
						});
						sublist.setSublistValue({
							id: 'custpage_customer',
							line: row,
