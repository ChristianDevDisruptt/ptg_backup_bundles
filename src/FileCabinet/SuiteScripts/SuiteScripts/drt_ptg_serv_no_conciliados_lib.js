 ...
 * @NApiVersion 2.x
 */
 define(['N/search', 'N/config', 'N/record', 'N/log', 'N/format'],

 function (search, config, record, log, format) {
	 ...
	 function getAllTransaction(params) {
		 ...
		 log.debug("params.mydate",params.mydate);

		 try {
			 var afilters = [];
			 afilters.push(['custrecord_ptg_ruta_sin_conciliar', search.Operator.ANYOF, params.vehiculo]);
			 afilters.push("AND");
			 afilters.push(['custrecord_ptg_sgcloc_opor_noconcil', search.Operator.ANYOF, "@NONE@"]);
			 afilters.push("AND");
			 afilters.push(['custrecord_ptg_sgcloc_fecha', search.Operator.WITHIN, params.mydateIni, params.mydateFin]);

			 log.audit({
				 title: 'afilters',
				 details: JSON.stringify(afilters)
			 });
			 //var acolumns = ['internalid', 'custrecord_ptg_ruta_sin_conciliar', 'custrecord_ptg_planta_sin_conciliar', 'custrecord_ptg_litros_sin_conciliar', 'custrecord_ptg_total_ser_sin_conciliar', 'custrecord_ptg_cliente_sin_conciliar'];
			 var acolumns = ['internalid', 'custrecord_ptg_ruta_sin_conciliar', 'custrecord_ptg_planta_sin_conciliar', {name: "custrecordptg_ubicacion_descr",join: "CUSTRECORD_PTG_PLANTA_SIN_CONCILIAR",label: "Nombre"}, 'custrecord_ptg_litros_sin_conciliar', 'custrecord_ptg_total_ser_sin_conciliar', 'custrecord_ptg_cliente_sin_conciliar'];
			 log.audit({
				 title: 'acolumns',
			 ...
			 schRecord.run().each(function (row) {
				 resultSearch.push({
					...
					nomplanta: row.getValue({name: "custrecordptg_ubicacion_descr",join: "CUSTRECORD_PTG_PLANTA_SIN_CONCILIAR",label: "Nombre"}),
					litros: row.getValue('custrecord_ptg_litros_sin_conciliar'),
					price: row.getValue('custrecord_ptg_total_ser_sin_conciliar'),
					cliente: row.getValue('custrecord_ptg_cliente_sin_conciliar')
				 });
				 return true;
			 });
	 ...
	 function getTranslate() {
		 ...
		 var label = {
			 ...
			 label1: strlang == true ? 'Vehículo' : 'Vehícle',
			 label2: strlang == true ? 'Fecha Inicio' : 'Start Date',
			 label3: strlang == true ? 'Fecha Fin' : 'End Date',
			 label4: strlang == true ? 'Busqueda Dirección' : 'Lookup Address',
			 label5: strlang == true ? 'Fecha Inicio' : 'Star Date',
			 labe16: strlang == true ? 'Fecha Fin' : 'End Date',
			 label7: strlang == true ? 'Subsidiaria' : 'Subsidiary',
			 label8: strlang == true ? 'Cliente' : 'Client',
			 ...
			 column7: strlang == true ? 'Precio' : 'Precio',
